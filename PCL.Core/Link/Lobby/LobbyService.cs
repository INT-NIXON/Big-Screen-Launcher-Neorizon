...
#else
                    LogWrapper.Error("Natayark ID 令牌已过期，请重新登录");
#endif
                }
                else
                {
...
#else
            LogWrapper.Error("大厅服务初始化失败，请检查网络连接。");
#endif
...
#else
            LogWrapper.Error("请先登录 Natayark ID 再使用大厅！");
#endif
            return false;
        }
...
#else
                LogWrapper.Error("在创建房间的时候遇到了问题，请查看日志并将此问题反馈给开发者！");
#endif
                return false;
            }
...
#else
            LogWrapper.Error("创建大厅失败，请检查日志或向开发者反馈。");
#endif
...
#else
            LogWrapper.Error("大厅编号格式不正确，请检查后再试！");
#endif
            await LeaveLobbyAsync().ConfigureAwait(false);
