...
public static class Basics
{
    ...
    private static MetadataModel _LoadMetadata()
    {
        try
        {
            var stream = Assembly.GetEntryAssembly()?.GetManifestResourceStream("PCL.metadata.json")
                         ?? Assembly.GetExecutingAssembly().GetManifestResourceStream("PCL.metadata.json");
            if (stream != null)
            {
                using var reader = new StreamReader(stream);
                return JsonSerializer.Deserialize<MetadataModel>(reader.ReadToEnd())
                       ?? new MetadataModel("BSLN", new LauncherVersionModel("0.1", "", 0, ""), []);
            }
        }
        catch { }
        return new MetadataModel("BSLN", new LauncherVersionModel("0.1", "", 0, ""), []);
    }
    ...
    {
        var resourceInfo = System.Windows.Application.GetResourceStream(new Uri($"pack://application:,,,/{path}", UriKind.Absolute));
...
#else
    public static Stream? GetResourceStream(string path) => null;
    public static string GetAppImagePath(string imageName) => imageName;
#endif
