...
partial class Lifecycle
{
    ...
    private static Thread? _waitForShutdownThread;

    /// <summary>
    /// [请勿调用] 开始等待关闭信号（非 WPF 模式）。由 OnInitialize 自动调用。
    /// </summary>
    private static void _BeginWaitForShutdown()
    {
        _waitForShutdownThread = new Thread(() =>
        {
            _ShutdownSignal.Wait();
            if (!HasShutdownStarted) _Exit(0);
        })
        {
            Name = "Lifecycle/WaitForShutdown",
            IsBackground = true
        };
        _waitForShutdownThread.Start();
    }
...
#else
    public static object CurrentApplication { get; set; } = null!;
#endif
