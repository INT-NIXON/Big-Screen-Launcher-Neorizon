...
#else
            if (showHint) LogWrapper.Warn("内存优化尚未结束，请稍等！");
#endif
            return false;
        }
...
#else
        if (showHint) LogWrapper.Info("正在进行内存优化");
#endif
...
#else
                        if (showHint) LogWrapper.Info($"内存优化结束，共优化 {diffStr}，目前可用内存 {afterStr}");
#endif
                    }
                    else
                    {
...
#else
            if (showHint) LogWrapper.Error("提权进程启动失败，请允许管理员权限以使用内存优化 - 内存优化失败");
#endif
            return false;
        }
        catch (Exception)
